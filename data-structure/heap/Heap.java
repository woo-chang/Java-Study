...
public class Heap {
  ...
  public boolean insert(Integer data) {
    ...
    while(this.moveUp(inserted_idx)) {
      ...
      Collections.swap(this.heapArray, parent_idx, inserted_idx);
  ...
  public boolean move_down(Integer popped_idx) {
    ...
    if (left_child_popped_idx >= this.heapArray.size()) {
      return false;
    } else if (right_child_popped_idx >= this.heapArray.size()) { // CASE2: 오른쪽 자식 노드만 없을 때
      if (this.heapArray.get(popped_idx) < this.heapArray.get(left_child_popped_idx)) {
        return true;
      } else {
        return false;
      }
    } else { // CASE3: 왼쪽/오른쪽 자식 노드가 있을 때
